...
// Vamos a realizar una función no tan optimizada para que no sea la misma que la solución de los profesores
var media = function (califications) {
    var suma = 0;
    califications.forEach(function (calification) { return suma += calification; });
    return suma / califications.length;
};
var summarizeClassRoom = function (studentList) { return studentList.map(function (element) { return ({
    name: element.name,
    notaMasAlta: Math.max.apply(Math, element.califications),
    notaMedia: media(element.califications).toPrecision(3),
}); }); };
