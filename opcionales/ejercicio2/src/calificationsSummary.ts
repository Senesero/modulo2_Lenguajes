...
const media = califications => {
  let suma = 0;
  califications.forEach((calification) => suma += calification);
  return suma / califications.length;
}
const summarizeClassRoom = (studentList: Array<any>) => studentList.map((element) => (
  {
    name: element.name,
    notaMasAlta: Math.max(...element.califications),
